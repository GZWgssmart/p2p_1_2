                    ...
                    <ul style="float:left;">
                        <c:forEach items="${obj}" var="d">
                            <li>
                                ...
                                <span class="pmain-titletype fb"><a href="javascript:;" onclick="del('${d.id}','/recommend/remove')">删除</a></span>
                            </li>
                        </c:forEach>
                        ...
                        <li>
                            <div align="center">
                                ...
                                <a href="/recommend/listByUid?pageNo=1">首页</a>
                                <c:choose>
                                    <c:when test="${page.pageNo - 1 > 0}">
                                        <a href="/recommend/listByUid?pageNo=${page.pageNo - 1}">上一页</a>
                                    </c:when>
                                    <c:when test="${page.pageNo - 1 <= 0}">
                                        <a href="/recommend/listByUid?pageNo=1">上一页</a>
                                    </c:when>
                                </c:choose>
                                <c:choose>
                                    <c:when test="${page.pages==0}">
                                        <a href="/recommend/listByUid?pageNo=${page.pageNo}">下一页</a>
                                    </c:when>
                                    <c:when test="${page.pageNo + 1 < page.pages}">
                                        <a href="/recommend/listByUid?pageNo=${page.pageNo + 1}">下一页</a>
                                    </c:when>
                                    <c:when test="${page.pageNo + 1 >= page.pages}">
                                        <a href="/recommend/listByUid?pageNo=${page.pages}">下一页</a>
                                    </c:when>
                                </c:choose>
                                <c:choose>
                                    <c:when test="${page.pages==0}">
                                        <a href="/recommend/listByUid?pageNo=${page.pageNo}">尾页</a>
                                    </c:when>
                                    <c:otherwise>
                                        <a href="/recommend/listByUid?pageNo=${page.pages}">尾页</a>
                                    </c:otherwise>
                                </c:choose>
