<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<%
    String path = request.getContextPath();
%>
<html>
<head>
    <title>Title</title>
</head>
<body>

</body>
<script src="<%=path%>/static/js/jquery.min.js?v=2.1.4"></script>
<script src="<%=path%>/static/js/bootstrap.min.js?v=3.3.5"></script>
<script src="<%=path%>/static/js/jquery.metisMenu.js"></script>
<script src="<%=path%>/static/js/jquery.slimscroll.min.js"></script>
<script src="<%=path%>/static/plugin/bootstrap/js/plugins/layer/layer.min.js"></script>
<script src="<%=path%>/static/plugin/bootstrap/js/hplus.min.js?v=4.0.0"></script>
<script src="<%=path%>/static/plugin/bootstrap/js/contabs.min.js"></script>
<script src="<%=path%>/static/js/pace.min.js"></script>
</html>
